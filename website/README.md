# zarathustra.kuber.studio

Static landing page for Zarathustra. No build step or runtime dependencies are required; serve this directory as the web root.
...
- The first viewport is a generated waveform field with Zarathustra's real recording overlay and no oversized app artwork. It does not depend on video or GIF playback.
...
- The feature bento demonstrates a paced, auto-scrolling Smart Cleanup stream, a three-row locale wall sourced from `SpeechTranscriber.supportedLocales`, and paired macOS-style Dictionary and Memory windows. The Dictionary demo supports adding, editing, and deleting words. The locale illustration currently reflects the 30 downloadable locales reported by the target Mac. Detailed Mail and Slack scenes alternate automatically to demonstrate active-window context, while the Hey Zarathustra response appears only after its section crosses the scroll threshold.
...
Serve `website/` at `https://qoodeng.github.io/zarathustra/`. The canonical URL, sitemap, structured data, and download links already target that domain.
